...
class trial_data():
    ...
    def calc_num_saccades_over_segment(self, saccade_speed_threshold, min_saccade_time_secs=0.020, start_percent=0, end_percent=100):
        '''
        Returns the number of saccades within a certain segment of the trial
        '''
        ...
        with warnings.catch_warnings():
            warnings.simplefilter("ignore", category=RuntimeWarning)
            self.results['num_saccades' + label_suffix] = self.processed_gaze_data['gaze_az_rel_car_filtered'].iloc[startIdx:endIdx]
    
        # need to interpolate velocity over where nans are
        try:
            interp_az = np.interp(np.arange(startIdx,endIdx), self.processed_gaze_data['gaze_az_rel_car_vel_filtered'].iloc[startIdx:endIdx].dropna().index, self.processed_gaze_data['gaze_az_rel_car_vel_filtered'].iloc[startIdx:endIdx].dropna())
            interp_el = np.interp(np.arange(startIdx,endIdx), self.processed_gaze_data['gaze_el_rel_head_vel_filtered'].iloc[startIdx:endIdx].dropna().index, self.processed_gaze_data['gaze_el_rel_head_vel_filtered'].iloc[startIdx:endIdx].dropna())

        except:
            interp_az = self.processed_gaze_data['gaze_az_rel_car_vel_filtered'].iloc[startIdx:endIdx]
            interp_el = self.processed_gaze_data['gaze_el_rel_head_vel_filtered'].iloc[startIdx:endIdx]
        
        # now merge the signals to do saccade detection on the result
        interp = np.sqrt(interp_az**2 + interp_el**2)
        
        # shortest time for a saccade is 20ms. Figure out how many samples this corresponds to
        saccade_samples = np.where(interp > saccade_speed_threshold)[0]
        #saccade_samples = self.processed_gaze_data['gaze_az_rel_car_vel_filtered'].iloc[startIdx:endIdx][self.processed_gaze_data['gaze_az_rel_car_vel_filtered'].iloc[startIdx:endIdx] > saccade_speed_threshold]
        
        min_samples = np.round(min_saccade_time_secs / np.mean(self.processed_gaze_data['pupilLabsTimeStamp'].diff()))
        
        #saccade_count = len(np.where(np.diff(saccade_samples.index.values) > min_samples)[0]) + 1 # because there will be 6 segments, 5 changes in index
        saccade_count = len(np.where(np.diff(saccade_samples) > min_samples)[0]) + 1
    ...
    def plot_fixations_on_road(self, showFig = False):
        
        '''
        This function plots the gaze projected onto the ground plane 
        '''
        
        if 'road_left_X' not in self.road_vertices.columns:
            self.calculate_road_edges()

        road_center = pd.concat((self.road_vertices[['xVertex_position', 'yVertex_position', 'zVertex_position']], pd.DataFrame(np.ones(len(self.road_vertices)).reshape(-1,1))),axis=1).values.reshape(-1,4)
        left_edge = pd.concat((self.road_vertices['road_left_X'], pd.DataFrame(np.zeros(len(self.road_vertices)).reshape(-1,1)), self.road_vertices['road_left_Z'], pd.DataFrame(np.ones(len(self.road_vertices)).reshape(-1,1))),axis=1).values.reshape(-1,4)
        right_edge = pd.concat((self.road_vertices['road_right_X'], pd.DataFrame(np.zeros(len(self.road_vertices)).reshape(-1,1)), self.road_vertices['road_right_Z'], pd.DataFrame(np.ones(len(self.road_vertices)).reshape(-1,1))),axis=1).values.reshape(-1,4)


        road_center_world = np.array([])
        road_edge_left_world = np.array([])
        road_edge_right_world = np.array([])
        for i in range(len(road_center)):
            road_center_world = np.concatenate((road_center_world, np.dot(self.road_data.values.reshape(4,4), road_center[i])),axis=0)
            road_edge_left_world = np.concatenate((road_edge_left_world, np.dot(self.road_data.values.reshape(4,4), left_edge[i])),axis=0)
            road_edge_right_world = np.concatenate((road_edge_right_world, np.dot(self.road_data.values.reshape(4,4), right_edge[i])),axis=0)

        road_center_world = road_center_world.reshape(-1,4)
        road_edge_left_world = road_edge_left_world.reshape(-1,4)
        road_edge_right_world = road_edge_right_world.reshape(-1,4)

        # plotting gaze on top of road. 
        # interp to length of self.processed_gaze_data for the math
        interp_head_tracker_R0C3 = np.interp(np.linspace(0,len(self.head_tracker),len(self.processed_gaze_data)), np.arange(0,len(self.head_tracker)), self.head_tracker['_cyclopeangaze_4x4_R0C3'])
        interp_head_tracker_R1C3 = np.interp(np.linspace(0,len(self.head_tracker),len(self.processed_gaze_data)), np.arange(0,len(self.head_tracker)), self.head_tracker['_cyclopeangaze_4x4_R1C3'])
        interp_head_tracker_R2C3 = np.interp(np.linspace(0,len(self.head_tracker),len(self.processed_gaze_data)), np.arange(0,len(self.head_tracker)), self.head_tracker['_cyclopeangaze_4x4_R2C3'])

        filtered_giw_dir_x = self.filter_spherical_coords('giw_dir_x')
        filtered_giw_dir_y = self.filter_spherical_coords('giw_dir_y')
        filtered_giw_dir_z = self.filter_spherical_coords('giw_dir_z')
        # interpolate up to right size
        filtered_giw_dir_x = np.interp(np.linspace(0,len(filtered_giw_dir_x),len(self.processed_gaze_data)), np.arange(0,len(filtered_giw_dir_x)), filtered_giw_dir_x)
        filtered_giw_dir_y = np.interp(np.linspace(0,len(filtered_giw_dir_y),len(self.processed_gaze_data)), np.arange(0,len(filtered_giw_dir_y)), filtered_giw_dir_y)
        filtered_giw_dir_z = np.interp(np.linspace(0,len(filtered_giw_dir_z),len(self.processed_gaze_data)), np.arange(0,len(filtered_giw_dir_z)), filtered_giw_dir_z)

        x = interp_head_tracker_R0C3  - interp_head_tracker_R1C3 * filtered_giw_dir_x / (-1*filtered_giw_dir_y) # might need to get rid of the -1 value here
        z = interp_head_tracker_R2C3  - interp_head_tracker_R1C3 * filtered_giw_dir_z / (-1*filtered_giw_dir_y)
            
        # for debugging, pretend gaze is on center of road
        #x = np.interp(np.linspace(0,len(road_center_world[:,0]),len(x)), np.arange(len(road_center_world)), road_center_world[:,0])
        #z = np.interp(np.linspace(0,len(road_center_world[:,2]),len(x)), np.arange(len(road_center_world)), road_center_world[:,2])


        # pupil labs has opposite y coordinate system and origin is at the head, so can't filter by y > or < 0

        
        # figure out how many points are within the road edges versus elsewhere and calculate percent
        proportion_gaze_on_road = []
        # finding the bounds of the road
        xvals = np.concatenate((road_edge_left_world[:,0], road_edge_right_world[:,0]),axis=0)
        lower_lim_x = np.min(xvals)
        upper_lim_x = np.max(xvals)
        zvals = np.concatenate((road_edge_left_world[:,2], road_edge_right_world[:,2]),axis=0)
        lower_lim_z = np.min(zvals)
        upper_lim_z = np.max(zvals)
        
        for i in range(len(x)):
            
            # check first if gaze is even within the range of the road
            if lower_lim_x < x[i] < upper_lim_x:
                if lower_lim_z < z[i] < upper_lim_z:
                    
                    # find the closest vertex on road
                    closest_vertex_idx_x = [np.abs(val - x[i]) for val in road_center_world[:,0]]
                    closest_vertex_idx_x = np.argmin(np.array(closest_vertex_idx_x))

                    if road_edge_left_world[closest_vertex_idx_x,0]<road_edge_right_world[closest_vertex_idx_x,0]:
                        if np.round(road_edge_left_world[closest_vertex_idx_x,0])-1 < x[i] < np.round(road_edge_right_world[closest_vertex_idx_x,0])+1:
                            # also check if z component is within road
                            if road_edge_left_world[closest_vertex_idx_x,2] < road_edge_right_world[closest_vertex_idx_x,2]:
                                if np.round(road_edge_left_world[closest_vertex_idx_x,2])-1 < z[i] < np.round(road_edge_right_world[closest_vertex_idx_x,2])+1:
                                    proportion_gaze_on_road.append(1)
                                else: 
                                    proportion_gaze_on_road.append(0)
                            else: 
                                if np.round(road_edge_right_world[closest_vertex_idx_x,2])-1 < z[i] < np.round(road_edge_left_world[closest_vertex_idx_x,2])+1:
                                    proportion_gaze_on_road.append(1)
                                else: 
                                    proportion_gaze_on_road.append(0)
                        else: 
                        ...
                        if np.round(road_edge_right_world[closest_vertex_idx_x,0])-1 < x[i] < np.round(road_edge_left_world[closest_vertex_idx_x,0])+1:
                            # also check if z component is within road
                            if road_edge_left_world[closest_vertex_idx_x,2] < road_edge_right_world[closest_vertex_idx_x,2]:
                                if np.round(road_edge_left_world[closest_vertex_idx_x,2])-1 < z[i] < np.round(road_edge_right_world[closest_vertex_idx_x,2])+1:
                                    proportion_gaze_on_road.append(1)
            ...
            # before you go on to the next pass of the for loop, correct for whether the point was within 2˚ of the nearest road edge to account for eye tracker error
            head_pos = np.array([interp_head_tracker_R0C3[i], interp_head_tracker_R1C3[i], interp_head_tracker_R2C3[i]])
            gaze_from_head = head_pos - np.array([x[i], 0, z[i]])
            # calculate head to closest road edge point
            # find the closest vertex on road
            closest_vertex_idx = np.concatenate((xvals.reshape(-1,1),zvals.reshape(-1,1)),axis=1) - np.array([x[i],z[i]])
            closest_vertex_idx = np.sqrt(closest_vertex_idx[:,0]**2 + closest_vertex_idx[:,1]**2)
            closest_vertex_idx = np.argmin(np.array(closest_vertex_idx))
            closest_point_on_road_edge = np.array([xvals[closest_vertex_idx], 0, zvals[closest_vertex_idx]])
            head_from_road_edge = head_pos - closest_point_on_road_edge
            theta = np.arccos(np.dot(gaze_from_head,head_from_road_edge) / (np.linalg.norm(gaze_from_head) * np.linalg.norm(head_from_road_edge))) * 180 / np.pi
            # only if the degrees between the vectors are less than 2 AND the gaze was classified off road will we change the value
            if (theta <  2) and (proportion_gaze_on_road[-1] == 0): # arbitrary threshold in degrees
                proportion_gaze_on_road = proportion_gaze_on_road[:-1]
                proportion_gaze_on_road.append(1) # assign gaze to be on road instead, assume person was looking at road   
                
        # save values for later use
        self.processed_gaze_data['gaze_on_ground_plane_x'] = x
        self.processed_gaze_data['gaze_on_ground_plane_z'] = z
        self.processed_gaze_data['binary_gaze_in_road_flag'] = proportion_gaze_on_road
        
        if showFig:
            plt.figure()
            plt.plot(road_center_world[:,0], road_center_world[:,2],'k--',markersize=2)
            plt.plot(road_edge_left_world[:,0], road_edge_left_world[:,2],'k-',markersize=2)
            plt.plot(road_edge_right_world[:,0], road_edge_right_world[:,2],'k-',markersize=2)
            plt.plot(x,z,'ro',markersize=1)
            plt.xlim(np.min(road_center_world[:,0])-self.experiment_settings['road_halfWidth']*2, np.max(road_center_world[:,0])+self.experiment_settings['road_halfWidth']*2)
            plt.ylim(np.min(road_center_world[:,2])-self.experiment_settings['road_halfWidth']*2, np.max(road_center_world[:,2])+self.experiment_settings['road_halfWidth']*2)
            plt.title('Gaze projected onto ground plane')
            ax = plt.gca()
            ax.set_aspect('equal')
            plt.show()
...
def run_all_subs_gd(data_parent_folder, start_at = 0, cbPatient = True, analyze_gaze = True, use_cached_data = False, exclude_list = []):

    # Gather subfolders and remove members of exlude_list
    data_folders = [f for f in os.scandir(data_parent_folder) if os.path.isdir(f) ]
    data_folder_paths = [f.path for f in data_folders if not f.name in exclude_list ]

    for subnum, subject_data_folder in enumerate(data_folder_paths):
        print(f'{subnum}: {subject_data_folder}')

    arg_list = [[data_folder, cbPatient, analyze_gaze, use_cached_data] for data_folder in data_folder_paths[start_at:]]

    # pool = multiprocessing.Pool(processes=3)

    # with multiprocessing.Pool() as pool:
    # print(f'** {subnum} / {len(data_folder_paths)} :  {os.path.split(subject_data_folder)[-1]}  **',flush=True)

    import multiprocessing as mp
    processes = [mp.Process(target=run_single_subject_gd, args=[d, cbPatient, analyze_gaze, use_cached_data]) for d in data_folder_paths[start_at:]]
    ...
    # tqdm(pool.imap(run_single_subject_gd, iterable=arg_list),total=len(data_folder_paths))

    result_list_tqdm = []
    # tqdm(pool.imap(func=func, iterable=argument_list), total=len(argument_list)):


    # for subnum, subject_data_folder in enumerate(data_folder_paths[start_at:]):
    #
    #     print(f'** {subnum} / {len(data_folder_paths)} :  {os.path.split(subject_data_folder)[-1]}  **')
    #
    #     run_single_subject_gd(cbPatient = cbPatient,
    #                           analyze_gaze = analyze_gaze,
...
def run_single_subject_gd(subject_data_folder, cbPatient = True, analyze_gaze = True, use_cached_data = False, gaze_contingent=False, estimate_head_trackers=False, session_num='S001'):
    ...
    this_sub = subject_data(subject_data_folder, cbPatient=cbPatient, analyze_gaze=analyze_gaze,
                            offset_car_xz=(-0.4304, 0), use_cached_data=use_cached_data, gaze_contingent=gaze_contingent, estimate_head_trackers=estimate_head_trackers, session_num=session_num)
    # change back to this if car is not visible: offset_car_xz=(-.4304, 0)
...
if __name__ == "__main__":
    ...
    people = ['cb10', 'cb41', 'cb36', 'cb13', 'cb5', 'cb42', 'cb37', 'cb12b', 'cb16', 'cb43', 'cb2', 'control1','control2','control3','control4','control5','control6','control7','control8','control9']

    for person in people:
        
        sub = subject_data(os.path.join(r'/Volumes/performlab2/data/2023-CB-Steering-Study1/cb/' + person),
                        cbPatient=False,
                        analyze_gaze=True,
                        estimate_head_trackers=False,
        ...
        for row_idx, row in sub.results.iterrows():
            trial = sub.get_trial_from_results(row)                            
                
	    # any trial specific functions can be performed here, average calculated over parts of trials, etc. See the subject and trial classes above for potential options.
